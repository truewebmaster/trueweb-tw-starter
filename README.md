...
- Добавлен базовый компонент Header.
- Настроен вывод логотипа через Custom Logo с резервным отображением названия сайта.
- Подключено главное меню WordPress (`wp_nav_menu`).
- Реализована адаптивная структура шапки.
- Добавлено мобильное меню с Overlay.
- Компонент Burger Button вынесен в отдельный шаблон.
- Подготовлена модульная архитектура JavaScript (`Header.js`).
- Добавлена анимация Header при прокрутке страницы.
- Подготовлена структура для подключения контактной информации и действий через настройки темы.
